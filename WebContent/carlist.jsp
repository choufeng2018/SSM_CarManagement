																...
																<TD style="PADDING-TOP: 8px" align=middle class="newsline"><table
																		...
																		<c:forEach items="${list}" var="bean">
																			...
																			<tr>
																				...
																				<td><img src="<%=basePath %>${bean.pic }"    height="70" border="0" /></td>
																				<td>${bean.pinpai }</td>
																				<td>${bean.xinghao }</td>
																				<td><span class="text-danger " ><bold >${bean.zujin }</bold></span></td>
																				<td>${bean.status }</td>

																				<td><a href="carview.do?id=${bean.id }">查看详情</a>
																					&nbsp;&nbsp;&nbsp; <c:if
																						test="${bean.status=='待租赁'}">
																						<input
														type="button" class="btn btn-danger" value="预定租车"
														onClick="if(confirm('确认预定?')){window.location.href='yudingadd.do?carid=${bean.id }'}return false;"/>
